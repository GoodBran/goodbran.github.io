...
cover_image: "/assets/images/query-graphql-with-ruby-cover.png"
---
...
It's just so natural that we will look for a gem for GraphQL query. And you actually do that, gem [graphql-client](https://github.com/github/graphql-client) would be your go to choice. It looks sophisticated and reliable enough, as it is produced by the github team, right?
...
Another problem is writing the graphql is hard, without the gem `graphql-client` to do the validation for you, you have no idea if you are writing the correct schema. However, this problem could be solved very easily, all you need is a smart code editor. I am using vscode here for example.
...
With this code structure, you store all you graphql queries in a separate folder. Query files are using `.gql` extension or `.graphql` extension. Next, just find a graphql extension and configure it. Below is a demo configuration.
