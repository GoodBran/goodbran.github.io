...
cover_image: "/assets/images/rails-engine-with-packwerk-cover.png"
---
...
The idea behind is that, master component handles all external interactions. And for specific task the master delegates to the child component. For child component, the folder structure is flat and properly name scoped.
...
Now, I want to enforce the boundary among component `identity` and `foundation`. So only controlled part of component is accessible. This time I will use the [packwerk](https://github.com/Shopify/packwerk) gem from Shopify.
...
Check the packwerk and it should all be fine, because we only have one package now.
...
## Give up on the folder collapse and following the rails naming convention
...
Also, remove the folder loading tricks we used before, no more tricks this time, let's run the packwerk command and see if it works now.
...
Now the table is ready, but we have no convenient interface to talk to the table still, I mean, there is no active record model yet. Let's now modify the `user` class.
...
In summary, if you have a somehow big project and you want to enforce a bit boundary between business component, using the folder structure could be a way to rescue. Anyway, that's it for now, will continue talk about the testing, tasks, generators, sorbot and more next (If I got time).
